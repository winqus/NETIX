...
export class MoviesController {
  ...
  public async createFromExternalSource(@Body() createFromExt: CreateMovieFromExternalSourceDTO) {
    try {
      ...
      const newMovieResult = await this.moviesService.createFromExternalSource(externalTitle);
      if (newMovieResult.isFailure) {
        throw new BadRequestException(newMovieResult.errorValue());
      }
      const newMovie = newMovieResult.getValue();

      this.logger.log(
        `Created new movie ${newMovie.uuid} by importing ${externalTitle.id} from external source ` +
